...
class MainApp(MDApp):
    ...
    def on_start(self):
        ...
        self.about.update_height(dp(900))
        ...
        if self.quiz_name in self.tiles:
            ...
            if self.card.children[0].text == 'UN ATTEMPTED':
                if self.score == 5:
                    ...
                    obj.text = 'Very Poor'
                    obj.meter = 'Images/Score/meter_1.png'
                    obj.star_1 = 'star-outline'
                    obj.star_2 = 'star-outline'
                    obj.star_3 = 'star-outline'
                    obj.star_4 = 'star-outline'
                    obj.star_5 = 'star-outline'
                    obj.star1_color = rgba(255, 255, 255, 255)
                    obj.star2_color = rgba(255, 255, 255, 255)
                    obj.star3_color = rgba(255, 255, 255, 255)
                    obj.star4_color = rgba(255, 255, 255, 255)
                    obj.star5_color = rgba(255, 255, 255, 255)
                    try:
                        is_attended = False
                        for ans in self.user_ans:
                            if ans != 'None':
                                is_attended = True
                        if (not self.skipped == '') and (is_attended):
    ...
    def on_scroll(self, scroll_y):
        if scroll_y - self.scroll_pos_y > 0:
            Animation(pos_hint={"center_y": -1}, duration=0.5).start(self.root.ids.res_hm)
            Animation(pos_hint={"center_y": -1}, duration=0.5).start(self.root.ids.res_hm_icon)
        else:
            Animation(pos_hint={"center_y": 0.06}, duration=1).start(self.root.ids.res_hm)
            Animation(pos_hint={"center_y": 0.06}, duration=1).start(self.root.ids.res_hm_icon)
